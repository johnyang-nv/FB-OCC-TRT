...
def main():
    ...
    for idx, data in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            # Prepare the grid
            grid = model.prepare_grid(
                inputs[-1], history_forward_augs, forward_augs, curr_to_prev_ego_rt, start_of_sequence
            )
        ...
        for key in inputs_.keys():
            ...
            if key != 'history_bev':
                ...
                print(f"Saved {key} as .dat to {dat_file_path}")
                if key in ['ranks_bev', 'interval_starts', 'indexes']:
                    print(f"Saved {key} as .npy to {npy_file_path}")
