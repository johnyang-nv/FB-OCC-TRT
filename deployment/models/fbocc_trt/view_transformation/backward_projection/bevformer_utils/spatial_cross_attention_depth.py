...
from mmdet3d.models.functions.multi_scale_deformable_attn import (
...
class DA_SpatialCrossAttention(BaseModule):
    ...
    def forward(self,
        ...
        # Prepare masks to avoid direct indexing
        for j in range(bs):
            for i in range(num_cams):
                index_query_per_img = indexes[j][i]
                num_valid = len(index_query_per_img)  # Number of valid indexes
                
                # Prepare a mask and index with it
                mask = torch.zeros(max_len, dtype=torch.bool, device=query.device)
                mask[:num_valid] = True
                
                # Assign based on the mask
                queries_rebatch[j, i, mask] = query[j, :index_query_per_img]
                bev_query_depth_rebatch[j, i, mask] = bev_query_depth[j, i, :index_query_per_img]
                reference_points_rebatch[j, i, mask] = reference_points_cam[j, :index_query_per_img]
    ...
    def forward(self,
        ...
        for j in range(bs):
            for i, reference_points_per_img in enumerate(reference_points_cam):   
                ...
                # index_query_per_img = indexes[j][i]
                # dims = torch.arange(len(index_query_per_img))
        ...
        # import numpy as np
        # fb_ = np.load('/FB-BEV/data/'+ 'bev_query_depth_rebatch.npy')
        # print('bev_query_depth_rebatch Disrecpancy', ((fb_ - bev_query_depth_rebatch.detach().cpu().numpy())**2).mean()) 
        # fb_ = np.load('/FB-BEV/data/'+ 'queries_rebatch.npy')
        # print('queries_rebatch Disrecpancy', ((fb_ - queries_rebatch.detach().cpu().numpy())**2).mean()) 
        ...
        # import numpy as np; fb_ = np.load('/FB-BEV/data/'+ 'queries_deform.npy')
        # print('queries Disrecpancy', ((fb_ - queries.detach().cpu().numpy())**2).mean()) 
        ...
        # import numpy as np; fb_ = np.load('/FB-BEV/data/'+ 'slots.npy')
        # print('bev_feat_b4_fuse Disrecpancy', ((fb_ - slots.detach().cpu().numpy())**2).mean()) 
...
class DA_MSDeformableAttentionTRT(DA_MSDeformableAttention): 
    def __init__(self,
        ...
        super(DA_MSDeformableAttentionTRT, self).__init__(embed_dims,
                                                          ...
                                                          norm_cfg,
                                                          init_cfg)
        from mmdet3d.models.utils.trt_register import TRT_FUNCTIONS
    ...
    def forward(self,
        ...
        # depth_weights = multi_scale_deformable_attn_pytorch(pred_img_depth.unsqueeze(2).contiguous(), 
        #                                             spatial_shapes[0:1],  
        #                                             depth_reference_points,
        #                                             depth_attention_weights
        #                                             ).reshape(bs, num_query, num_Z_anchors, -1)
        ...
        # level_start_index - torch.int64
        # sampling_locations - torch.float32
        # attention_weights torch.float32
